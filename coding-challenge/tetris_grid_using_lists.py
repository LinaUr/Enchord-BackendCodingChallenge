...
class TetrisGridUsingLists(TetrisGrid):
    """
    The Tetris grid is represented as a list of lists of booleans.
    Each inner list represents a row, and each boolean represents whether the cell is occupied.
    """
    _FULL_ROW: int = 10 # The number of 1s in a full row
    ...
    def insert_block_from_top(self, block: ListBlock, left_most_col: int) -> None:
    ...
    def _insert_block(self, top_left_corner_in_grid: Tuple[int, int], block: ListBlock) -> None:
    ...
    def _is_valid_block_placement(self, top_left_corner_in_grid: Tuple[int, int], block: ListBlock) -> bool:
        grid_row_idx, grid_col_idx = top_left_corner_in_grid
        ...
        for block_row_idx in range(block.height):
            for block_col_idx in range(block.width):
                ...
                if is_grid_field_occupied and is_block_field_occupied:
                    # block is overlapping with another block
                    return False
        return True